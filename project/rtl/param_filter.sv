module param_filter
#(
    parameter int N = 16, // обязательно кратное 4
    parameter int BITS_W=40,
    parameter int MULT_LATENCY = 0  // Задержка умножителя
)(
    input logic reset_i,
    input logic clk_i,
    input logic enable_i,
    input logic signed [15:0] signal_i,
    input logic signed [15:0] taps_i [0:N-1], // можете поменять порядок на N-1:0
    input logic order_sel_i, // 0 -> N, 1 -> N/2
    input logic clk2x_i,
    output logic signed [BITS_W-1:0] signal_o   
);

    localparam int HALF = N / 2;
    localparam int TREE_DEPHT = $clog2(HALF);
    localparam int NUM_PAIRS = HALF / 2;
    
    localparam int ADDER_TREE_LATENCY = (NUM_PAIRS <= 1) ? 0 : $clog2(NUM_PAIRS);
    localparam int TOTAL_LATENCY = MULT_LATENCY + ADDER_TREE_LATENCY + 1;
    
    logic signed [BITS_W-1:0] upper_ff  [0:HALF-1];
    logic signed [BITS_W-1:0] upper_next [0:HALF-1];
    logic signed [BITS_W-1:0] lower_ff  [0:HALF-2];
    logic signed [BITS_W-1:0] lower_next [0:HALF-2];
    logic signed [BITS_W-1:0] spc_ff;
    logic signed [BITS_W-1:0] spc_next;
    
    always_comb begin: c_shift_logic
        upper_next[0] = signal_i;
        for (int i = 1; i < HALF; i++) begin
            upper_next[i] = upper_ff[i-1];
        end
        
        if (order_sel_i == '0) begin
            lower_next[0] = upper_ff[HALF-1];
        end else begin
            lower_next[0] = spc_ff;
        end
        for (int i = 1; i < HALF-1; i++) begin
            lower_next[i] = lower_ff[i-1];
        end
        
        spc_next = signal_i;
    end: c_shift_logic
    
    always_ff @(posedge clk_i) begin: p_upper
        if (reset_i) begin
            for (int i = 0; i < HALF; i++) upper_ff[i] <= '0;
        end else if (enable_i) begin
            upper_ff <= upper_next;
        end
    end: p_upper
    
    always_ff @(posedge clk_i) begin: p_lower
        if (reset_i) begin
            for (int i = 0; i < HALF-1; i++) lower_ff[i] <= '0;
        end else if (enable_i) begin
            lower_ff <= lower_next;
        end
    end: p_lower
    
    always_ff @(posedge clk_i) begin: p_spc
        if (reset_i) begin
            spc_ff <= '0;
        end else if (enable_i) begin
            spc_ff <= spc_next;
        end
    end: p_spc
    
    logic signed [BITS_W-1:0] samp_upper [0:HALF-1];
    logic signed [BITS_W-1:0] samp_lower [0:HALF-1];
    logic signed [15:0]       coef_upper [0:HALF-1];
    logic signed [15:0]       coef_lower [0:HALF-1];
    
    logic signed [BITS_W-1:0] tree_in_ue [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] tree_in_uo [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] tree_in_le [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] tree_in_lo [0:NUM_PAIRS-1];
    
    logic signed [15:0] coef_ue [0:NUM_PAIRS-1];
    logic signed [15:0] coef_uo [0:NUM_PAIRS-1];
    logic signed [15:0] coef_le [0:NUM_PAIRS-1];
    logic signed [15:0] coef_lo [0:NUM_PAIRS-1];
    
    always_comb begin: c_mux
        samp_upper[0] = signal_i;
        for (int k = 1; k < HALF; k++) begin
            samp_upper[k] = upper_ff[k-1];
        end
        
        if (order_sel_i == '0) begin
            samp_lower[0] = upper_ff[HALF-1];
        end else begin
            samp_lower[0] = spc_ff;
        end
        for (int k = 1; k < HALF; k++) begin
            samp_lower[k] = lower_ff[k-1];
        end
        
        for (int k = 0; k < HALF; k++) begin
            if (order_sel_i == '0) begin
                coef_upper[k] = taps_i[k];
                coef_lower[k] = taps_i[k + HALF];
            end else begin
                coef_upper[k] = taps_i[k];
                coef_lower[k] = taps_i[k];
            end
        end
        
        for (int k = 0; k < NUM_PAIRS; k++) begin
            tree_in_ue[k] = samp_upper[2*k];
            tree_in_uo[k] = samp_upper[2*k + 1];
            tree_in_le[k] = samp_lower[2*k];
            tree_in_lo[k] = samp_lower[2*k + 1];
            
            coef_ue[k] = coef_upper[2*k];
            coef_uo[k] = coef_upper[2*k + 1];
            coef_le[k] = coef_lower[2*k];
            coef_lo[k] = coef_lower[2*k + 1];
        end
    end: c_mux
    
    logic signed [31:0] prod_raw_ue [0:NUM_PAIRS-1];
    logic signed [31:0] prod_raw_uo [0:NUM_PAIRS-1];
    logic signed [31:0] prod_raw_le [0:NUM_PAIRS-1];
    logic signed [31:0] prod_raw_lo [0:NUM_PAIRS-1];
    
    logic signed [BITS_W-1:0] prod_ue [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] prod_uo [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] prod_le [0:NUM_PAIRS-1];
    logic signed [BITS_W-1:0] prod_lo [0:NUM_PAIRS-1];
    
    generate
        for (genvar k = 0; k < NUM_PAIRS; k++) begin : g_mult_ue
            multiplier16 u_mult (
                .clk_i   (clk_i),
                .enable_i(enable_i),
                .reset_i (reset_i),
                .a_i     (tree_in_ue[k][15:0]),
                .b_i     (coef_ue[k]),
                .m_o     (prod_raw_ue[k])
            );
        end
        for (genvar k = 0; k < NUM_PAIRS; k++) begin : g_mult_uo
            multiplier16 u_mult (
                .clk_i   (clk_i),
                .enable_i(enable_i),
                .reset_i (reset_i),
                .a_i     (tree_in_uo[k][15:0]),
                .b_i     (coef_uo[k]),
                .m_o     (prod_raw_uo[k])
            );
        end
        for (genvar k = 0; k < NUM_PAIRS; k++) begin : g_mult_le
            multiplier16 u_mult (
                .clk_i   (clk_i),
                .enable_i(enable_i),
                .reset_i (reset_i),
                .a_i     (tree_in_le[k][15:0]),
                .b_i     (coef_le[k]),
                .m_o     (prod_raw_le[k])
            );
        end
        for (genvar k = 0; k < NUM_PAIRS; k++) begin : g_mult_lo
            multiplier16 u_mult (
                .clk_i   (clk_i),
                .enable_i(enable_i),
                .reset_i (reset_i),
                .a_i     (tree_in_lo[k][15:0]),
                .b_i     (coef_lo[k]),
                .m_o     (prod_raw_lo[k])
            );
        end
    endgenerate
    
    // Расширение знака с 32 бит до BITS_W для adder_tree
    always_comb begin: c_mult_extend
        for (int k = 0; k < NUM_PAIRS; k++) begin
            prod_ue[k] = BITS_W'($signed(prod_raw_ue[k]));
            prod_uo[k] = BITS_W'($signed(prod_raw_uo[k]));
            prod_le[k] = BITS_W'($signed(prod_raw_le[k]));
            prod_lo[k] = BITS_W'($signed(prod_raw_lo[k]));
        end
    end: c_mult_extend
    
    logic signed [BITS_W-1:0] sum_ue;
    logic signed [BITS_W-1:0] sum_uo;
    logic signed [BITS_W-1:0] sum_le;
    logic signed [BITS_W-1:0] sum_lo;

    generate
        adder_tree #(
            .NUM_INPUTS(NUM_PAIRS),
            .BITS_W(BITS_W),
            .LEVEL(0)
        ) u_tree_ue (
            .clk_i    (clk_i),
            .enable_i (enable_i),
            .reset_i  (reset_i),
            .inputs_i (prod_ue),
            .summ_o   (sum_ue)
        );

        adder_tree #(
            .NUM_INPUTS(NUM_PAIRS),
            .BITS_W(BITS_W),
            .LEVEL(0)
        ) u_tree_uo (
            .clk_i    (clk_i),
            .enable_i (enable_i),
            .reset_i  (reset_i),
            .inputs_i (prod_uo),
            .summ_o   (sum_uo)
        );

        adder_tree #(
            .NUM_INPUTS(NUM_PAIRS),
            .BITS_W(BITS_W),
            .LEVEL(0)
        ) u_tree_le (
            .clk_i    (clk_i),
            .enable_i (enable_i),
            .reset_i  (reset_i),
            .inputs_i (prod_le),
            .summ_o   (sum_le)
        );

        adder_tree #(
            .NUM_INPUTS(NUM_PAIRS),
            .BITS_W(BITS_W),
            .LEVEL(0)
        ) u_tree_lo (
            .clk_i    (clk_i),
            .enable_i (enable_i),
            .reset_i  (reset_i),
            .inputs_i (prod_lo),
            .summ_o   (sum_lo)
        );
    endgenerate


    logic order_sel_delayed;

    delay #(
        .D (TOTAL_LATENCY),
        .W (1)
    ) u_delay_order (
        .reset_i  (reset_i),
        .clk_i    (clk_i),
        .enable_i (enable_i),
        .signal_i (order_sel_i),
        .signal_o (order_sel_delayed)
    );

    logic signed [BITS_W-1:0] y_current_next;
    logic signed [BITS_W-1:0] y_prev_next;
    logic signed [BITS_W-1:0] y_current_ff;
    logic signed [BITS_W-1:0] y_prev_ff;

    always_comb begin: c_final_sum
        if (order_sel_delayed == '0) begin
            y_current_next = sum_ue + sum_uo + sum_le + sum_lo;
            y_prev_next    = '0;
        end else begin
            y_current_next = sum_ue + sum_uo;
            y_prev_next    = sum_le + sum_lo;
        end
    end: c_final_sum

    always_ff @(posedge clk_i) begin: p_final_regs
        if (reset_i) begin
            y_current_ff <= '0;
            y_prev_ff    <= '0;
        end else if (enable_i) begin
            y_current_ff <= y_current_next;
            y_prev_ff    <= y_prev_next;
        end
    end: p_final_regs

    // PSC
    logic psc_sel_ff;

    always_ff @(posedge clk2x_i) begin: p_psc
        if (reset_i) begin
            psc_sel_ff <= '0;
        end else if (enable_i) begin
            psc_sel_ff <= ~psc_sel_ff;
        end
    end: p_psc

    logic signed [BITS_W-1:0] psc_out;
    assign psc_out = (psc_sel_ff == '0) ? y_current_ff : y_prev_ff;

    logic signed [BITS_W-1:0] signal_o_n_ff;   // N на clk_i
    logic signed [BITS_W-1:0] signal_o_n2_ff;  // N/2 на clk2x_i

    always_ff @(posedge clk_i) begin: p_out_n
        if (reset_i) begin
            signal_o_n_ff <= '0;
        end else if (enable_i) begin
            signal_o_n_ff <= y_current_ff;
        end
    end: p_out_n

    always_ff @(posedge clk2x_i) begin: p_out_n2
        if (reset_i) begin
            signal_o_n2_ff <= '0;
        end else if (enable_i) begin
            signal_o_n2_ff <= psc_out;
        end
    end: p_out_n2

    assign signal_o = order_sel_delayed ? signal_o_n2_ff : signal_o_n_ff;
    
endmodule 
